...
python_test() {
	tc-export CC CXX
	# we can always rely on parallel workflow through numpy for python < 3.5
	# We use the flags used upstream in travis.yml. Failure to do so seems
	# to results in errors.
	CFLAGS="-O0 -ggdb -Wall -Wextra"  "${PYTHON}" runtests.py -vv --work-dir "${BUILD_DIR}"/tests -j $(makeopts_jobs) \
