...
src_prepare() {
	...
	# replace $SAGE_ROOT/local with $SAGE_LOCAL
	sed -i "s:\$SAGE_ROOT/local:\$SAGE_LOCAL:g" sage-gdb-ipython \
	...
	cd "${ROOT_S}"
	epatch "${FILESDIR}"/${PN}-5.6-gentooify-startup-script.patch.bz2
...
src_install() {
	...
	if use debug ; then
		...
		dobin sage-gdb-ipython sage-gdb-commands
