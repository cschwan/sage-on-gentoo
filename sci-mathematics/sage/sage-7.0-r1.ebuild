...
IUSE="latex testsuite debug X manifolds ${DOC_USE} ${SAGE_USE}"
LINGUAS_USEDEP=""
for X in ${LANGS} ; do
...
CDEPEND="dev-libs/gmp:0=
	...
	>=sci-libs/mpfi-1.5.1
	~sci-libs/pynac-0.6.0[${PYTHON_USEDEP}]
...
python_prepare() {
	...
	if use manifolds ; then
		...
		epatch "${FILESDIR}"/${PN}-7.0-manifolds.patch
	fi
	...
	# ATLAS independence
	epatch "${FILESDIR}"/${PN}-7.0-blas.patch
	...
	epatch "${FILESDIR}"/${PN}-7.0-singular_extra_arg.patch
	...
	epatch "${FILESDIR}"/${PN}-6.10-sources.patch
