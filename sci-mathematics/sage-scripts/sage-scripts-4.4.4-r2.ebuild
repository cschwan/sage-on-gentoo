...
src_prepare() {
	epatch "${FILESDIR}"/${PN}-4.4.4-remove-useless-options.patch
	epatch "${FILESDIR}"/${PN}-4.4.4-remove-useless-variables.patch
}
...
src_install() {
	...
	dobin sage-banner sage-cleaner sage-env sage-eval sage-ipython \
		sage-location sage-maxima.lisp sage-make_relative sage-native-execute \
		sage-run sage-sage sage-spkg sage-startuptime.py || die
