...
src_prepare() {
	epatch "${FILESDIR}"/${PN}-4.5-remove-useless-options.patch
	epatch "${FILESDIR}"/${PN}-4.5-remove-useless-variables.patch
...
src_install() {
	...
	dobin sage-banner sage-cleaner sage-env sage-eval sage-ipython \
		sage-maxima.lisp sage-make_relative sage-native-execute sage-run \
